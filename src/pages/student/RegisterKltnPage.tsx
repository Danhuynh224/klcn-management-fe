...
import { createKltn, getMyRegistrations } from '../../services/registrations.api';
...
const registerKltnSchema = z.object({
  ...
  fieldName: z.string().min(1, 'Vui lòng nhập lĩnh vực'),
  companyName: z.string().optional(),
  supervisorId: z.union([z.string(), z.number()]).refine(Boolean, 'Vui lòng chọn GVHD'),
  termId: z.union([z.string(), z.number()]).refine(Boolean, 'Vui lòng chọn đợt'),
});
...
export default function StudentRegisterKltnPage() {
  ...
  const { control, handleSubmit, setValue, formState } = useForm<RegisterKltnValues>({
    ...
    <div className="page-stack">
      ...
      {!isEligible ? (
        <Alert
          ...
          description="Hệ thống cần xác nhận bạn đã có hồ sơ BCTT ở trạng thái BCTT_PASSED trước khi mở form KLTN."
        />
      ) : null}
